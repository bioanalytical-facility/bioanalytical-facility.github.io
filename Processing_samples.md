...
3.  For Partial Least Squares analysis, such as PLS-DA, OPLS-DA, PLS-R, OPLS-R, etc, the `?? variable` must be set appropriately:
...
6.  The "Multivariate analysis result window" opens. Click `Save plot properties` to save the results to a `*.txt` file. Refer to the bullets below for useful info about the result window:
    ...
    -   Click `VIP` to see the VIP features. **NOTE: For (O)PLS-DA, these features are with respect to the samples with a `?? variable` of 0.** Change the `?? variable` of the different cohorts to see the results with respect to a different cohort.
